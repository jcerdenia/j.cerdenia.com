...
export const unpackFile = (name) => {
  if (!unpackedFiles[name]) {
    ...
    if (name.includes("/") && !name.endsWith("index.md")) {
      ...
      data.draft = index.draft || data.draft;
      data.parent = { slug: parentSlug, title: index.title };
      fullTitle = `${data.parent.title}: ${fullTitle}`;
    }

    if (process.env.ENV === "dev" && data.draft) {
    ...
    unpackedFiles[name] = { content, data };
  }

  return unpackedFiles[name];
};
